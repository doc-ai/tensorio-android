 ...
 * TensorIO
 *
 * Created by Philip Dow on 02/19/2021
...
public class ScalarConverter implements ai.doc.tensorio.core.data.Converter, Converter {

    @Override
    public ByteBuffer createBackingBuffer(@NonNull LayerDescription description, int batchSize) {
        boolean quantized = ((ScalarLayerDescription)description).isQuantized();
        int length = ((ScalarLayerDescription)description).getLength();
    ...
    public ByteBuffer toByteBuffer(@NonNull byte[] bytes, @NonNull LayerDescription description, @Nullable ByteBuffer cache) throws IllegalArgumentException {
        ...
        ScalarLayerDescription vectorLayerDescription = (ScalarLayerDescription) description;
