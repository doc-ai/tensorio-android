 ...
 * although separate denormalization biases may be required for each of the RGB channels.
 *
...
public abstract class PixelDenormalizer {
    /**
     * A `PixelNormalizer` transforms a pixel value in the range `[0,255]`
    ...
    /**
     * A PixelDenormalizer that applies a scaling factor and equal bias to each pixel channel.
     */

    public static PixelDenormalizer PixelDenormalizerSingleBias(float scale, float bias) {
        return new PixelDenormalizer() {
            @Override
    ...
    /**
     * A PixelDenormalizer that applies a scaling factor and different biases to each pixel channel.
     */

    public static PixelDenormalizer PixelDenormalizerPerChannelBias(float scale, float redBias, float greenBias, float blueBias) {
        return new PixelDenormalizer() {
            @Override
            public int denormalize(float value, int channel) {
                switch (channel) {
    ...
    public static PixelDenormalizer PixelDenormalizerZeroToOne() {
        float scale = 255.0f;
        return PixelDenormalizerSingleBias(scale, 0.0f);
    }
    ...
    public static PixelDenormalizer PixelDenormalizerNegativeOneToOne() {
        float scale = 255.0f / 2.0f;
        float bias = 1f;
        return PixelDenormalizerSingleBias(scale, bias);
    }
